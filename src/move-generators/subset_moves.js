...
var scramble_subset = (function () {
	...
	function $getDRtoDL(obj) {
		...
		for (i = 11; i >= 0; --i) {
			if (4 <= obj.ep[i] && obj.ep[i] <= 6) {
				idxA = idxA + (Cnk)[i][r--];
				t = 1 << obj.ep[i];
				idxB = idxB + bitCount(mask & t - 1) * [1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800, 39916800, 479001600][2 - r];
				mask = (mask | t);
			}
		}
		return idxA * 6 + idxB;
	...
	function $getEdgePerm(obj) {
		...
		for (i = 10; i >= 0; --i) {
			t = 1 << obj.ep[i];
			idx += bitCount(m_0 & t - 1) * ([1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800, 39916800, 479001600])[11 - i];
			m_0 |= t;
		}
		return idx;
	}
	...
	function set8Perm(arr, idx) {
		...
		val = 1985229328;
		for (i = 0; i < 7; ++i) {
			p = ([1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800, 39916800, 479001600])[7 - i];
			v = ~~(idx / p);
			idx = idx - v * p;
			v <<= 2;
			arr[i] = (val >> v & 7);
...
function subset_moves(config){
	...
	return {
		"Edge": scramble_subset.getEdgeScramble,
		"Corner": scramble_subset.getCornerScramble,
		"LL": scramble_subset.getLLScramble,
		"CMLL": scramble_subset.getCMLLScramble,
		"LSLL": scramble_subset.getLSLLScramble,
		"ZBLL": scramble_subset.getZBLLScramble,
		"2GLL": scramble_subset.get2GLLScramble,
		"PLL": scramble_subset.getPLLScramble,
		"ZZLS": scramble_subset.getZZLSScramble,
		"F2L": scramble_subset.getF2LScramble
	}[config.subset]().split(" ").filter(function(x){return x.length > 0});
}

module.exports = {subset_moves};